...
const EventAnalytics: React.FC = () => {
  ...
  const [regStatusFilter, setRegStatusFilter] = useState<'all' | 'attended' | 'registered' | 'pending'>('all');
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {activeTab === 'registrations' && (() => {
                ...
                const filteredRegs = analytics.recentRegistrations.filter(reg => {
                  ...
                  const matchesStatus = regStatusFilter === 'all' ||
                    (regStatusFilter === 'attended' && reg.status === 'attended') ||
                    (regStatusFilter === 'registered' && reg.status === 'registered' && reg.approvalStatus === 'approved') ||
                    (regStatusFilter === 'pending' && reg.approvalStatus === 'pending');
                  return matchesSearch && matchesStatus;
                });
                const totalPages = Math.max(1, Math.ceil(filteredRegs.length / REG_PAGE_SIZE));
                const safePage = Math.min(regPage, totalPages);
                const pagedRegs = filteredRegs.slice((safePage - 1) * REG_PAGE_SIZE, safePage * REG_PAGE_SIZE);
                const attendedCount = analytics.recentRegistrations.filter(r => r.status === 'attended').length;
                const registeredCount = analytics.recentRegistrations.filter(r => r.status === 'registered' && r.approvalStatus === 'approved').length;
                const pendingCount = analytics.recentRegistrations.filter(r => r.approvalStatus === 'pending').length;
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{analytics.recentRegistrations.length} total &middot; {attendedCount} attended &middot; {registeredCount} registered &middot; {pendingCount} pending</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 mb-5">
                    <div className="relative flex-1">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-9 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 bg-gray-50 transition-all"
                      />
                      {regSearch && (
                        <button onClick={() => { setRegSearch(''); setRegPage(1); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                    ...
                    <div className="flex gap-2">
                      {[
                        { key: 'all' as const, label: 'All', count: analytics.recentRegistrations.length },
                        { key: 'attended' as const, label: 'Attended', count: attendedCount },
                        { key: 'registered' as const, label: 'Registered', count: registeredCount },
                        { key: 'pending' as const, label: 'Pending', count: pendingCount },
                      ].map(f => (
                        <button
                          key={f.key}
                          onClick={() => { setRegStatusFilter(f.key); setRegPage(1); }}
                  ...
                  {regSearch && (
                    <p className="text-xs text-gray-500 mb-3">
                      Showing {filteredRegs.length} result{filteredRegs.length !== 1 ? 's' : ''} for "{regSearch}"
                    </p>
                  )}
                  ...
                  {pagedRegs.length > 0 ? (
                    <div className="space-y-3">
                      {pagedRegs.map((reg, index) => (
                        <motion.div
                          key={`${reg.userName}-${reg.registeredAt}-${index}`}
                          initial={{ opacity: 0, x: -20 }}
                          animate={{ opacity: 1, x: 0 }}
                            ...
                            <div>
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                {reg.approvalStatus === 'pending' && (
                                  <span className="inline-flex items-center px-2 py-0.5 bg-yellow-50 text-yellow-600 text-xs font-medium rounded-full">
                                    <Clock className="w-3 h-3 mr-1" />
                                    Pending
                                  </span>
                                )}
                                {reg.approvalStatus === 'rejected' && (
                                  <span className="inline-flex items-center px-2 py-0.5 bg-red-50 text-red-600 text-xs font-medium rounded-full">
                    ...
                    <div className="text-center py-12 text-gray-500">
                      {regSearch || regStatusFilter !== 'all' ? (
                        <>
                          ...
                          <button onClick={() => { setRegSearch(''); setRegStatusFilter('all'); setRegPage(1); }} className="mt-2 text-sm text-emerald-600 hover:text-emerald-700 font-medium">
                            Clear filters
                          </button>
                        </>
                      ) : (
                  ...
                  {totalPages > 1 && (
                    <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100">
                      <p className="text-sm text-gray-500">
                        Showing {((safePage - 1) * REG_PAGE_SIZE) + 1}–{Math.min(safePage * REG_PAGE_SIZE, filteredRegs.length)} of {filteredRegs.length}
                      </p>
